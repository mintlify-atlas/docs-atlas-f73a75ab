---
title: "POST /vote/ — vote or unvote on a post"
sidebarTitle: "POST /vote/"
...
| `404 Not Found` | The target post does not exist, **or** (when `dir: 0`) the authenticated user has not voted on this post. |
| `409 Conflict` | The authenticated user has already voted on this post (when `dir: 1`). |
